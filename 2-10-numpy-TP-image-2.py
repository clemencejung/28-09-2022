...
# %% {"scrolled": false}
#1
f=open("rgb-codes.txt")
dico={}
for line in f.readlines():
    if line=="":
        continue
    couleur = line.replace("\n","").split(" ")
    dico[couleur[0]]=[int(couleur[1]),int(couleur[2]),int(couleur[3])]
    
print(dico)

listecouleurs=list(dico.items())
print(listecouleurs)
...
#3
def patchwork(liste):
    n=int(np.sqrt(len(liste)))+1
    A=255*np.ones((n,n,3),dtype=np.uint8)
    for k in range(len(liste)):
        q,r=divmod(k,n)
        A[q][r][0]=int(liste[k][1][0])
        A[q][r][1]=int(liste[k][1][1])
        A[q][r][2]=int(liste[k][1][2])
    plt.imshow(A)    
...
# %% {"scrolled": true}
patchwork(listecouleurs)

# %% {"scrolled": false}
#4
n=np.random.randint(0,len(listecouleurs))
L=[]
for k in range (n):
    L.append(listecouleurs[np.random.randint(0,len(listecouleurs))])     
patchwork(L)    
...
# %%
#6
A=patchwork(listecouleurs)
plt.imsave("patchwork.jpg",A)
